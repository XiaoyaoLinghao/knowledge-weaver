...
_SYS_PROMPT = """你是知识抽取助手。下面给你一段“会话摘要”，请从中抽取结构化原子事实，\
仅输出一个 JSON 数组，不要任何额外文字、解释或代码围栏。
...
- 同一概念只输出一条，type 必须一致；summary 一句话（≤60 字）；aliases 可省略或为空数组。
- 若摘要无实质知识内容（仅心跳/运维/无意义），输出 []。"""
...
def backfill_text(text: str, extract_fn) -> tuple[str, int]:
...
def backfill_file(path: str, extract_fn, *, dry_run: bool = False) -> int:
    with open(path, "r", encoding="utf-8") as f:
        text = f.read()
    new_text, n = backfill_text(text, extract_fn)
